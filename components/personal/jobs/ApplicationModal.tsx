...
export const ApplicationModal = ({
  ...
  const fetchResumes = async () => {
    ...
    try {
      const { isLocalhost } = useUserStore.getState();
      const token = sessionStorage.getItem("accessToken");

      const headers =
        token && isLocalhost ? { Authorization: `Bearer ${token}` } : {};

      const response = await apiClient.get("/api/personal/resume/list", {
        headers,
      ...
      >
        ...
        <div className="flex items-start justify-between p-4">
          <div>
            ...
            <h3 className="mt-1 text-xl font-medium text-blue-500">
              {jobTitle}
            </h3>
          </div>
          <button
            ...
            <div className="p-4">
              {loading ? (
                ...
                resumes.map((resume) => (
                  <div
                    key={resume.id}
                    className={`mb-4 text-sm text-gray-800 cursor-pointer ${
                      selectedResume?.id === resume.id ? "bg-blue-100" : ""
                    }`}
