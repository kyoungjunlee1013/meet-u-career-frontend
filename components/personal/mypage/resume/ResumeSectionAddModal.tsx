...
export function ResumeSectionAddModal({ isOpen, onClose, onAdd }: ResumeSectionAddModalProps) {
  const [title, setTitle] = useState("")
  const [template, setTemplate] = useState("project")
  const [fields, setFields] = useState<string[]>(["name", "organization", "startDate", "endDate", "description"])
  ...
  const handleTemplateChange = (value: string) => {
    ...
    switch (value) {
      case "project":
        setFields(["name", "role", "startDate", "endDate", "description", "technologies"])
        break
      case "research":
        setFields(["title", "institution", "date", "description", "publication"])
        break
      case "publication":
      ...
      case "custom":
        ...
        break
      default:
        setFields(["name", "organization", "startDate", "endDate", "description"])
    }
  }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
              ...
              <div className="mt-2 space-y-2">
                {getAvailableFields(template).map((field) => (
                  <div key={field.value} className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor={`field-${field.value}`} className="cursor-pointer">
                      {field.label}
                    </Label>
                  </div>
                ))}
          ...
          <div className="border-t p-4 flex justify-end space-x-2">
            ...
            <Button className="bg-blue-600 hover:bg-blue-700" onClick={handleSubmit} disabled={!title.trim()}>
              추가
            </Button>
          </div>
        </div>
