...
export function ResumeSectionEditorCard({
  section,
  onContentUpdate,
}: ResumeSectionEditorCardProps) {
      ...
      {list.map((item: any, idx: number) => (
        <div key={idx} className="bg-gray-50 rounded-lg border p-6 mb-6">
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-4">
            <div>
              <Label htmlFor={`organization-${idx}`}>
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange(idx, "organization", e.target.value)
                }
              />
            </div>
            <div>
      ...
      {list.map((item: any, idx: number) => (
        <div key={idx} className="bg-gray-50 rounded-lg border p-6 mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      handleChange(idx, "isCurrentlyWorking", e.target.checked)
                    }
                    className="accent-blue-600 w-4 h-4"
                  />
                  <Label
                    htmlFor={`current-${idx}`}
                    className="ml-1 text-sm font-normal cursor-pointer"
                  >
                    현재 재직중
                  </Label>
                </div>
          ...
          <div className="mb-4">
            ...
            <Textarea
              id={`description-${idx}`}
              placeholder="담당했던 주요 업무와 성과를 입력하세요"
              value={item.description}
              onChange={(e) => handleChange(idx, "description", e.target.value)}
              rows={3}
            />
      ...
      {list.map((item: any, idx: number) => (
        <div key={idx} className="bg-gray-50 rounded-lg border p-6 mb-6">
          ...
          <div className="mb-4">
            ...
            <Textarea
              id={`description-${idx}`}
              placeholder="활동/경험 관련 설명을 입력하세요"
              value={item.description}
              onChange={(e) => handleChange(idx, "description", e.target.value)}
              rows={3}
            />
...
function PortfolioSection({
  content,
  onContentUpdate,
  resumeFileMode = false,
}: {
  content: any;
  onContentUpdate: (content: any) => void;
  resumeFileMode?: boolean;
}) {
  ...
  return (
    <div>
      <Label className="block mb-2">
        {resumeFileMode ? "이력서 파일 업로드" : "포트폴리오 파일 추가"}
      </Label>
      <input
        ref={fileInputRef}
        type="file"
      ...
      <div className="space-y-4">
        ...
        {fileList.map((file: any, idx: number) => (
          <div
            key={idx}
            className="flex items-center justify-between bg-gray-50 rounded border p-4"
          >
            <div>
...
function CoverLetterSection({
  ...
  useEffect(() => {
    ...
    setError(null);
    apiClient
      .get("/api/personal/coverletter/list")
      .then((res) => {
  ...
  const selected = content?.selectedCoverLetterId
    ? coverLetters.find(
        (cl: any) => String(cl.id) === String(content.selectedCoverLetterId)
      )
    : null;
  return (
    <div>
        ...
        <Select
          ...
          onValueChange={(value) => {
            const selected = coverLetters.find(
              (cl: any) => String(cl.id) === String(value)
            );
            if (selected) {
              onContentUpdate({
