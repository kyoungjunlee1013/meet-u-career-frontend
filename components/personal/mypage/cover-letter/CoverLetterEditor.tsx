...
export function CoverLetterEditor({
  ...
  const [sections, setSections] =
    useState<CoverLetterSection[]>(defaultSections);
      ...
      const fetchCoverLetterData = async () => {
        try {
          ...
          const res = await apiClient.get(
            `/api/personal/coverletter/view?id=${id}`
          );
          ...
          toast({
            title: err.message || "상세 데이터를 불러오지 못했습니다.",
            variant: "destructive",
          });
        } finally {
          setIsLoading(false);
  ...
  const handleAddSection = (sectionTitle: string) => {
    setSections((prev) => {
      ...
      return [...prev, { id: newId, sectionTitle, content: "" }];
    });
  ...
  const handleSectionContentUpdate = (
    sectionId: string,
    field: keyof CoverLetterSection,
    value: string
  ) => {
    setSections((prev) =>
        ...
        prev.map((s) =>
          s.id === sectionId
            ? {
                ...
                revisedContent:
                  data.revisedContent || "AI가 제안한 수정 예시입니다.",
                error: undefined,
              }
            : s
        )
