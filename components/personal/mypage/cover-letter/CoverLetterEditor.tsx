...
export function CoverLetterEditor({
    ...
    if (isEditMode && id) {
      ...
      const fetchCoverLetterData = async () => {
        try {
          ...
          setSections([
            {
              ...
              originalContent:
                "백엔드 개발자로서의 역량을 발휘하고 성장하기 위해 지원하게 되었습니다. 귀사의 기술 스택과 제가 보유한 기술이 일치하여 즉시 기여할 수 있을 것으로 생각합니다.",
              feedback:
                "지원 동기가 명확하게 표현되어 있지만, 조금 더 구체적인 내용이 추가되면 좋을 것 같습니다.",
              revisedContent:
          ...
          toast({
            title: "데이터 로드 실패",
            description: "자기소개서 데이터를 불러오는 중 오류가 발생했습니다.",
            variant: "destructive",
          });
        } finally {
          setIsLoading(false);
  ...
  const handleRequestFeedback = async (sectionId: string) => {
    ...
    try {
      ...
      setSections((prevSections) =>
        prevSections.map((s) =>
          s.id === sectionId ? { ...s, originalContent: s.content } : s
        )
      );
      ...
      setSections((prevSections) =>
        prevSections.map((s) =>
          s.id === sectionId
            ? { ...s, feedback, revisedContent, isApplied: false }
            : s
        )
      );
