...
function getInterviewTypeLabels(typeBitmap?: number) {
  if (!typeBitmap) return [];
  return interviewTypeLabels.filter(t => (typeBitmap & t.value) !== 0).map(t => t.label);
}

export function ReviewDetailModal({ isOpen, onClose, review }: { isOpen: boolean; onClose: () => void; review: ReviewDetail }) {
  if (!review) return null;
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle className="text-xl">면접 리뷰 상세</DialogTitle>
        ...
        <div className="flex flex-col md:flex-row gap-6 my-4">
          ...
          <div className="flex-1 space-y-3">
            ...
            <div>
              ...
              <div className="whitespace-pre-line bg-gray-50 rounded p-2 mt-1 text-sm text-gray-800 min-h-[40px]">{review.questionsAsked || "-"}</div>
            </div>
            <div>
              <span className="font-medium">면접 팁:</span>
              <div className="whitespace-pre-line bg-gray-50 rounded p-2 mt-1 text-sm text-gray-800 min-h-[40px]">{review.interviewTip || "-"}</div>
            </div>
          </div>
        </div>
        <DialogFooter>
