      ...
      <DialogContent className="max-w-xl">
        ...
        <div className="flex flex-col md:flex-row gap-6 my-4">
          ...
          <div className="flex-1 space-y-3 text-sm text-gray-800">
            <div><span className="font-medium">면접 연월:</span> {review.interviewYearMonth || "-"}</div>
            <div><span className="font-medium">전반적 평가:</span> {typeof review.rating === "number" ? ratingMap[review.rating] : "-"}</div>
            <div><span className="font-medium">난이도:</span> {review.difficulty >= 1 && review.difficulty <= 5 ? difficultyMap[review.difficulty - 1] : "-"}</div>
            <div><span className="font-medium">면접 유형:</span> {getInterviewTypeLabels(review.interviewType).join(", ") || "-"}</div>
            <div><span className="font-medium">면접 인원:</span> {typeof review.interviewParticipants === "number" ? participantMap[review.interviewParticipants] : "-"}</div>
            <div><span className="font-medium">합격 여부:</span> {typeof review.result === "number" ? resultMap[review.result] : "-"}</div>
