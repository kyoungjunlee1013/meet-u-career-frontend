...
interface OfferProps {
  offer: {
    ...
    deadline: string
    description: string
    status: '검토중' | '수락함' | '거절함' // 변경된 타입: string으로 명시
  }
  onActionComplete?: (nextTab: '수락함' | '거절함') => void
}

export function OfferCard({ offer, onActionComplete }: OfferProps) {
  const [status, setStatus] = useState<'검토중' | '수락함' | '거절함'>(offer.status)
  ...
  const handleAction = async (type: 'approve' | 'reject') => {
    try {
      ...
      onActionComplete?.(nextStatus) // 탭 이동 유도
    } catch (err) {
      alert('처리에 실패했습니다.')
      console.error(err)
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "수락함":
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-600">✅ 수락함</span>
      case "거절함":
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-600">❌ 거절함</span>
      default:
        return null
    }
  }
