...
import {
  ...
  type ScheduleItem,
} from "../../personal/schedule/Calendar";
          ...
          {(() => {
            ...
            const dedupedSchedules = (() => {
              const mapped = schedules.map((ev) => ({
                ...
                _baseKey: `${ev.title
                  .replace(/(접수 시작|접수 마감|스크랩 마감)/g, "")
                  .trim()}|${ev.companyName || (ev.company?.name ?? "")}|${
                  ev.endDateTime
                }`,
              }));
              ...
              <PublicUpcomingEventsSidebar
                events={(() => {
                  ...
                  const deduped = mapped.filter(
                    (event, idx, arr) =>
                      arr.findIndex((e) => e._baseKey === event._baseKey) ===
                      idx
                  );
                  // 5. 내부 필드 제거
                  return deduped.map(
                    ({ _priority, _baseKey, ...rest }) => rest
                  );
                })()}
              />
            ) : (
